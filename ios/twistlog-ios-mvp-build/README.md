...
2. Use SwiftUI.
3. Target iOS 17+ if possible, so SwiftData is available.
4. Add the Swift files from `SwiftUI-Skeleton/TwistLog` into the project.
5. Start with the in-memory `AppStore` included here.
6. Replace `AppStore` with SwiftData after the flow feels right.
